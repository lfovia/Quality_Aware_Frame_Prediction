...
class GeneratorModel:
    ...
    def define_graph(self):
        ...
        with tf.name_scope('generator'):
            ...
            with tf.name_scope('train'):
                ...
                lgn_gen_test = Straight_ness_double_derivatives_deriv_gen3(self.scale_preds_test[3],128,8)
                ...
                lgn_gen_input = Straight_ness_double_derivatives_deriv_input_frames(self.input_frames_train,128,8)
                lgn_gen_input_test = Straight_ness_double_derivatives_deriv_input_frames(self.input_frames_test,128,8)
                ############################
                tf_lgn_input_stack = tf.stack([lgn_gen_input[0],lgn_gen_input[1],lgn_gen_input[2],lgn_gen_input[3]],3)
                input_tolinear = tf_lgn_input_stack[:,:,:,:,0]
                tf_lgn_input_stacktest = tf.stack([lgn_gen_input_test[0],lgn_gen_input_test[1],lgn_gen_input_test[2],lgn_gen_input_test[3]],3)
                input_tolineartest = tf_lgn_input_stacktest[:,:,:,:,0]
                ...
                #pdb.set_trace()
                self.discrimator_lossgt = discriminator_2(input_tolinear,output1gt,reuse=False)
                #pdb.set_trace()
                self.discrimator_lossgt_test = discriminator_2(input_tolineartest,output1gen_test,reuse=True)
                self.discrimator_lossgt_train = discriminator_2(input_tolinear,output1gen_train,reuse=True)
                ...
                self.global_loss2 = self.global_loss + 0.1* self.loss_g
    ...
    def train_step(self, batch,batch2, discriminator=None):
        ...
        for jj in range(5):
            _ =  self.sess.run([self.d_train2],feed_dict=feed_dict)
                          
                           
            
        _,_, global_loss, global_psnr_error, global_sharpdiff_error, global_step, summaries = \
